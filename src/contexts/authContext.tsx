...
const AuthProvider: React.FunctionComponent = ({ children }) => {
  ...
  async function signUpWithEmail(username: string, email: string, password: string, isAdmin: "0" | "1", url: string) {
    try {
      await cognito.signUpUserWithEmail(username, email, password, isAdmin, url)
    } catch (err) {
      throw err
    }
  }
